...
const CircleDrawingTest: React.FC = () => {
  ...
  const [timeRemaining, setTimeRemaining] = useState<number | null>(30); // 30 seconds for circle drawing
  ...
  useEffect(() => {
    ...
    const handleStrokeStart = (event: CustomEvent) => {
      ...
      // Store stroke start time for later use
      (currentStroke as any).startTime = strokeStartTime;
    };
    ...
    const handleStrokeEnd = (event: CustomEvent) => {
      ...
      if (currentStroke.length > 0) {
        ...
        const strokeStartTime = (currentStroke as any).startTime || strokeEndTime;
  ...
  const handleCompleteTask = async () => {
    ...
    try {
      const completionData = {
        ...
        elapsedTime: timeElapsed * 1000, // Convert to milliseconds
        strokes: allStrokes,
        canvasSize: {
        ...
        userInteractions: {
          pauseCount: 0, // Could track this if needed
          clearCount: 0, // Could track this if needed
          undoCount: 0   // Could track this if needed
        }
      };

      await completeTaskAndNavigate(completionData, 'square_drawing');
    } catch (error) {
  ...
  const instructionsContent = (
    <>
      <p>Draw a circle as round and smooth as possible</p>
      <p>Try to make it as close to a perfect circle as you can</p>
      <p>Take your time and focus on smooth movements</p>
        ...
        >
          ...
          <DrawingArea>
            ...
            <div style={{ 
              ...
              {hasStarted ? 'Draw your circle here...' : 'Click "Start Task" button to begin drawing'}
            </div>
            {stylusCapabilities && (
              <div style={{ 
                position: 'absolute', 
          ...
          <ButtonContainer>
            <Button to="/tasks" $primary={false}>
              <ArrowLeft size={20} />
              Back to Tasks
            </Button>
            <Button to="/test/square_drawing" $primary={true}>
              Next Task
              <ArrowRight size={20} />
            </Button>
          </ButtonContainer>
...
export default CircleDrawingTest;
