...
const WordMemoryTest: React.FC = () => {
  ...
  const wordsToMemorize = ['APPLE', 'CAR', 'BOOK', 'TREE', 'HOUSE'];
  ...
  const getStatusText = () => {
    ...
    if (isWriting) return 'Writing words from memory...';
    if (phase === 'memorize') return 'Memorizing words...';
  ...
  const instructionsContent = (
    <>
      <p>Remember these 5 words: APPLE, CAR, BOOK, TREE, HOUSE</p>
      <p>After 30 seconds, write them down from memory</p>
  ...
  return (
    <TestContainer>
      <div className="container">
        <TestHarness title="Word Memory"
          instructions={instructionsContent}
          onQuit={handleQuitTest}
          onPause={handlePauseToggle} step={1} totalSteps={21}
        >
          {phase === 'memorize' && showWords && (
            <MemoryPhase>
              <MemoryText>{wordsToMemorize.join(' ')}</MemoryText>
              <CountdownTimer>
          ...
          <WritingArea>
            ...
            <div style={{ 
              ...
              {phase === 'memorize' ? 'Memorize the words above...' : 
               hasStarted ? 'Write the words from memory...' : 'Click "Start Task" button to begin'}
            </div>
          ...
          <ButtonContainer>
            ...
            <Button to="/test/sentence_memory" $primary={true}>
              Next Task
              <ArrowRight size={20} />
            </Button>
          </ButtonContainer>
