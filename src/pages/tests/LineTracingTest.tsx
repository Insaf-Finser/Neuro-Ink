...
const StatusIndicator = styled.div<{ $status: 'waiting' | 'tracing' | 'completed' }>`
  display: flex;
  align-items: center;
  gap: 12px;
  padding: 16px;
  ...
  background: ${props => 
    ...
    props.$status === 'tracing' ? '#fef3c7' : '#f3f4f6'
  };
  border: 2px solid ${props => 
    props.$status === 'completed' ? '#10b981' :
    props.$status === 'tracing' ? '#f59e0b' : '#d1d5db'
  };
`;
...
const StatusText = styled.div<{ $status: string }>`
  ...
  color: ${props => 
    ...
    props.$status === 'tracing' ? '#d97706' : '#6b7280'
  };
`;
...
const LineTracingTest: React.FC = () => {
  ...
  const [timeRemaining, setTimeRemaining] = useState<number | null>(30); // 30 seconds for line tracing
  ...
  useEffect(() => {
    ...
    const drawWavyLine = () => {
      ...
      for (let x = startX; x <= endX; x += 2) {
        const y = centerY + Math.sin((x - startX) * 0.02) * 20;
        if (x === startX) {
          ctx.moveTo(x, y);
        } else {
          ctx.lineTo(x, y);
  ...
  const getStatusText = () => {
    ...
    if (isTracing) return 'Tracing in progress...';
    if (hasStarted) return 'Continue tracing...';
    return 'Ready to start';
  };

  const instructionsContent = (
    <>
      <p>Follow the wavy line exactly</p>
      <p>Stay on the line as closely as possible</p>
      <p>Keep your movement smooth</p>
  ...
  return (
    <TestContainer>
      <div className="container">
        <TestHarness title="Line Tracing"
        ...
        >
          ...
          <Instructions>
            <InstructionsText>
              Follow the wavy line exactly as shown above
            </InstructionsText>
          </Instructions>
          ...
          <DrawingArea>
            ...
            <div style={{ 
              ...
              {hasStarted ? 'Trace the line here...' : 'Click "Start Task" button to begin tracing'}
            </div>
            {stylusCapabilities && (
              <div style={{ 
                position: 'absolute', 
          ...
          <ButtonContainer>
            <Button to="/test/complex_figure_copy" $primary={false}>
              <ArrowLeft size={20} />
              Previous Task
            </Button>
            <Button to="/test/name_memory" $primary={true}>
              Next Task
              <ArrowRight size={20} />
            </Button>
          </ButtonContainer>
...
export default LineTracingTest;
