...
evaluateDrawing();
  ...
  const evaluateDrawing = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const validation = validateDrawing({ type: 'line' }, strokes, canvasSize);
  ...
  const instructions = (
    <Instructions>
      <InstructionText>• Follow the wavy line exactly</InstructionText>
      <InstructionText>• Stay on the line as closely as possible</InstructionText>
      <InstructionText>• Keep your movement smooth</InstructionText>
      ...
      >
        ...
        <div style={{ position: 'relative' }}>
          <DrawingCanvas
            ref={canvasRef}
            disabled={!hasStarted}
            placeholder={hasStarted ? (timeRemaining === 0 ? 'Time\'s up! Test completed.' : 'Trace the line here...') : 'Tap canvas to start test'}
            onTap={handleCanvasTap}
            onStrokeStart={handleStrokeStart}
            onStrokeEnd={handleStrokeEnd}
            referenceShape={{ type: 'line' }}
          />
