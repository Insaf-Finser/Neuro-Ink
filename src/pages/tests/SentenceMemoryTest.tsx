...
const SentenceMemoryTest: React.FC = () => {
  ...
  const [timeRemaining, setTimeRemaining] = useState<number | null>(120); // 120 seconds total
  ...
  const sentenceToMemorize = "The quick brown fox jumps over the lazy dog";
      ...
      const timer = setTimeout(() => {
        setMemorizeTime(prev => {
          if (prev <= 1) {
            ...
            setTimeRemaining(90); // 90 seconds to write
            return 0;
          }
          return prev - 1;
        });
  ...
  const getStatusText = () => {
    ...
    if (isWriting) return 'Writing sentence from memory...';
    if (phase === 'memorize') return 'Memorizing sentence...';
  ...
  const instructionsContent = (
    <>
      <p>Remember this sentence: "The quick brown fox jumps over the lazy dog"</p>
      <p>After 30 seconds, write it from memory</p>
      <p>Try to get every word correct</p>
      <p><strong>Time Limit: 120 seconds total</strong></p>
    </>
  );
  ...
  return (
    <TestContainer>
      <div className="container">
        <TestHarness title="Sentence Memory"
          instructions={instructionsContent}
          onQuit={handleQuitTest}
          onPause={handlePauseToggle} step={1} totalSteps={21}
        >
          {phase === 'memorize' && showSentence && (
            <MemoryPhase>
              <MemoryText>"{sentenceToMemorize}"</MemoryText>
              <CountdownTimer>
          ...
          <WritingArea>
            ...
            <div style={{ 
              ...
              {phase === 'memorize' ? 'Memorize the sentence above...' : 
               hasStarted ? 'Write the sentence from memory...' : 'Click "Start Task" button to begin'}
            </div>
          ...
          <ButtonContainer>
            <Button to="/test/word_memory" $primary={false}>
              <ArrowLeft size={20} />
              Previous Task
            </Button>
...
export default SentenceMemoryTest;
