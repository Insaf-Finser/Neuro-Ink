...
const PentagonDrawingTest: React.FC = () => {
  ...
  const evaluateDrawing = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const validation = validateDrawing({ type: 'pentagon' }, strokes, canvasSize);
      setValidationResult(validation);

      const totalTimeMs = Math.max(1, timeElapsed * 1000);
      const analysis = analyzeTest('pentagonDrawing', strokes, canvasSize, totalTimeMs, validation.accuracy);
      setAiResult(analysis.aiResult);

      await saveTestResult({
        testName: 'pentagonDrawing',
  ...
  const instructions = (
    <Instructions>
      <InstructionText>• Draw a five-sided shape (pentagon)</InstructionText>
      <InstructionText>• Make all sides equal length</InstructionText>
      <InstructionText>• Try to make it as regular as possible</InstructionText>
  ...
  return (
    <Container>
      <TestHarness
        title="Pentagon Drawing Test"
        step={4}
        totalSteps={21}
        instructions={instructions}
        isComplete={timeRemaining === 0 && hasStarted}
        onRetry={clearCanvas}
        onNext={() => navigate('/test/spiral_drawing')}
...
export default PentagonDrawingTest;
