...
const PatternCompletionTest: React.FC = () => {
  ...
  const evaluateDrawing = async () => {
    ...
    try {
      ...
      // Placeholder cognitive score; could be refined if we analyze repetition accuracy
  ...
  const instructions = (
    <Instructions>
      <InstructionText>• Look at the pattern shown above the drawing area</InstructionText>
      <InstructionText>• Continue the pattern in the same way</InstructionText>
      <InstructionText>• Complete at least 3 more elements of the pattern</InstructionText>
        ...
        <StatusCard $status={getStatus()}>
          ...
          <StatusText $status={getStatus()}>
            {timeRemaining === 0 ? 'Time\'s up!' :
             ...
             hasStarted ? 'Continue the pattern...' : 'Ready to start'}
          </StatusText>
        </StatusCard>
