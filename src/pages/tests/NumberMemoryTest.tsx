...
const NumberMemoryTest: React.FC = () => {
  ...
  const numbersToMemorize = ['7', '4', '2', '9', '1', '6'];
  ...
  const getStatusText = () => {
    ...
    if (isWriting) return 'Writing numbers from memory...';
    if (phase === 'memorize') return 'Memorizing numbers...';
  ...
  const instructionsContent = (
    <>
      <p>Remember this sequence: 7-4-2-9-1-6</p>
      <p>After 30 seconds, write it from memory</p>
      <p>Keep the numbers in the same order</p>
  ...
  return (
    <TestContainer>
      <div className="container">
        <TestHarness title="Number Memory"
          instructions={instructionsContent}
          onQuit={handleQuitTest}
          onPause={handlePauseToggle} step={1} totalSteps={21}
        >
          {phase === 'memorize' && showNumbers && (
            <MemoryPhase>
              <MemoryText>{numbersToMemorize.join('-')}</MemoryText>
              <CountdownTimer>
          ...
          <WritingArea>
            ...
            <div style={{ 
              ...
              {phase === 'memorize' ? 'Memorize the numbers above...' : 
               hasStarted ? 'Write the numbers from memory...' : 'Click "Start Task" button to begin'}
            </div>
          ...
          <ButtonContainer>
            <Button to="/test/name_memory" $primary={false}>
              <ArrowLeft size={20} />
              Previous Task
            </Button>
            <Button to="/test/rapid_writing" $primary={true}>
              Next Task
              <ArrowRight size={20} />
            </Button>
          </ButtonContainer>
