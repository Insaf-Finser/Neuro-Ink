...
const SquareDrawingTest: React.FC = () => {
  ...
  const evaluateDrawing = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const validation = validateDrawing({ type: 'square' }, strokes, canvasSize);
  ...
  const handleNext = () => {
    const nextTaskId = getNextTaskId('square_drawing', currentDisease);
    if (nextTaskId) {
  ...
  const instructions = (
    <Instructions>
      <InstructionText>• Draw a square with equal sides</InstructionText>
      <InstructionText>• Make sure all angles are 90 degrees</InstructionText>
      <InstructionText>• Keep the lines straight and connected</InstructionText>
      ...
      >
        ...
        <div style={{ position: 'relative' }}>
          <DrawingCanvas
            ref={canvasRef}
            disabled={!hasStarted}
            placeholder={hasStarted ? (timeRemaining === 0 ? 'Time\'s up! Test completed.' : 'Draw your square here...') : 'Tap canvas to start test'}
            onTap={handleCanvasTap}
            onStrokeStart={handleStrokeStart}
            onStrokeEnd={handleStrokeEnd}
            referenceShape={{ type: 'square' }}
          />
...
export default SquareDrawingTest;
